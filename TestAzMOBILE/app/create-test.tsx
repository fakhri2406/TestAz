    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor }]}>
            <View style={styles.section}>
                <Text style={[styles.title, { color: textColor }]}>{translations.addNewTest}</Text>
                <TextInput
                    style={[styles.input, { backgroundColor, color: textColor, borderColor: iconColor }]}
                ...
                <View style={styles.premiumContainer}>
                    <Text style={[styles.premiumLabel, { color: textColor }]}>{translations.premiumTest}</Text>
                    <Switch
                        value={newTest.isPremium}
                        onValueChange={(value) => setNewTest(prev => ({ ...prev, isPremium: value }))}
                    />
                </View>
            </View>

            <View style={styles.section}>
                <Text style={[styles.title, { color: textColor }]}>{translations.question}</Text>
                <TextInput
                    style={[styles.input, { backgroundColor, color: textColor, borderColor: iconColor }]}
                ...
                <Text style={[styles.subtitle, { color: textColor }]}>{translations.answerOptions}</Text>
                {currentQuestion.options.map((option, index) => (
                    <View key={index} style={styles.optionContainer}>
                        <TextInput
                            style={[styles.input, styles.optionInput, { backgroundColor, color: textColor, borderColor: iconColor }]}
                        ...
                        >
                            <Text style={[styles.correctButtonText, { color: option.isCorrect ? backgroundColor : tintColor }]}>
                                {option.isCorrect ? '✓' : '○'}
                            </Text>
                        </TouchableOpacity>
                    </View>
                ...
                <TouchableOpacity
                    style={[styles.button, styles.addQuestionButton, { backgroundColor: tintColor }]}
                    onPress={addQuestion}
                >
                    <Ionicons name="add-circle-outline" size={24} color={backgroundColor} />
            ...
            {newTest.questions.length > 0 && (
                <View style={styles.section}>
                    ...
                    {newTest.questions.map((question, index) => (
                        <View key={index} style={[styles.questionItem, { backgroundColor, borderColor: iconColor }]}>
                            <Text style={[styles.questionText, { color: textColor }]}>{question.text}</Text>
                            <Text style={[styles.optionText, { color: textColor }]}>
                                {translations.correctAnswer}: {question.options.find(opt => opt.isCorrect)?.text}
                            </Text>
                            <TouchableOpacity
                            ...
                            >
                                ...
                                <Text style={styles.removeButtonText}>{translations.delete}</Text>
                            </TouchableOpacity>
                        </View>
                    ))}
                </View>
            ...
            <View style={styles.buttonContainer}>
                <TouchableOpacity 
                    ...
                    onPress={createTest}
                    disabled={loading || !newTest.title || !newTest.description || newTest.questions.length === 0}
                >
                ...
                <TouchableOpacity 
                    style={[styles.button, styles.backButton, { backgroundColor, borderColor: tintColor }]}
                    onPress={() => router.back()}
                >
            ...
            {error && (
                <View style={[styles.errorContainer, { backgroundColor: '#f8d7da' }]}>
                    <Text style={[styles.errorTitle, { color: '#721c24' }]}>{translations.error}:</Text>
